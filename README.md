...
DeerFlow includes an optional, user-isolated document RAG subsystem. It stores source files under the user's DeerFlow data directory, ingests Markdown, text, PDF, DOCX, PPTX, and XLSX documents asynchronously, and combines SQLite FTS5 retrieval with embedding cosine search using reciprocal-rank fusion and MMR. Knowledge bases are independent from conversational Memory: documents preserve source/version/citation semantics, while Memory continues to hold extracted user facts.

The subsystem is disabled by default, so DeerFlow starts without an embedding model or API key. To enable it, set `knowledge.enabled: true` in `config.yaml`, configure `knowledge.embedding`, then restart Gateway because parser, embedding, index, and worker objects are startup-only. The REST surface begins at `/api/knowledge-bases`; thread and custom-agent bindings determine what the `knowledge_search` agent tool may retrieve. The tool never accepts a `user_id` or arbitrary knowledge-base ID, and full source metadata is returned in its tool artifact for citation rendering.

See [the RAG design](docs/plans/2026-07-31-rag-design.md) for architecture, storage, security, API, and rollout details.
